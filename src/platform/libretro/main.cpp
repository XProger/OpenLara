...
void retro_run(void)
{
   ...
   input_poll_cb();
	...
	static  bool  L3Presed = false;
	static  bool  R3Presed = false;
   ...
   for (size_t i = 0; i < 2; i++)
   {
	  ...
	  if (ret & (1 << RETRO_DEVICE_ID_JOYPAD_L3)) //Save
	  {
         //Input::setJoyDown(i, JoyKey::ik5, true);
		 printf("Input::setDown(ik5, 1)d \n");
		 if(!L3Presed)
			Game::quickSave();
		L3Presed = true;
	  }
      else
        // Input::setJoyDown(i, JoyKey::ik5, false);
		L3Presed = false;
	  
	  if (ret & (1 << RETRO_DEVICE_ID_JOYPAD_R3)) //Load
	  {
        // Input::setJoyDown(i, JoyKey::ik9, true);
		printf("Input::setDown(ik9, 1)d \n");
		 if(!R3Presed)
			Game::quickLoad(false);
		R3Presed = true;
	  }
      else
        // Input::setJoyDown(i, JoyKey::ik9, false);
		R3Presed = false;
...
bool retro_serialize(void *data, size_t size)
{
	...
	if (!Game::level || TR::isTitleLevel(Game::level->level.id) || TR::isCutsceneLevel(Game::level->level.id)) {
        return false;
    }
    //level->saveGame(level->level.id, true, false);
	
	//saveGame(TR::LevelID id, bool checkpoint, bool updateStats)
    ...
    removeSaveSlot((Game::level->level.id), true); // remove checkpoints and level saves
	SaveSlot slotsave = Game::level->createSaveSlot((Game::level->level.id), true);
    saveSlots.push(slotsave);
	saveSlots.sort();
    
	saveResult = SAVE_RESULT_WAIT;
    //UI::showHint(STR_HINT_SAVING, 60.0f);
    //int count;
    //uint8 *savecount = writeSaveSlots(count);
	memcpy( data, &slotsave.size, sizeof(uint32));
	memcpy( ((uint8*)data+sizeof(uint32)), slotsave.data, count);
	delete[] slotsave.data;
...
bool retro_unserialize(const void *data, size_t size)
{
	...
	memcpy( &slotload.size , data,  sizeof(uint32));
	
	slotload.data =  slotload.size
	
	
    int slot = getSaveSlot(Game::level->level.id, true);
		
	printf("quickLoad slot :%d \n", slot);
	
	
    if (slot == -1) {
        slot = getSaveSlot(Game::level->level.id, false);
    }

    if (slot > -1) {
        Game::level->loadGame(slot);
    }	
